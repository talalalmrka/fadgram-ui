...
const srcdoc = computed(() => {
    ...
    return `
<!DOCTYPE html>
<html lang="en" class="${darkMode.value ? 'dark' : ''}">
<head>
  <meta charset="UTF-8">
  <meta
    name="viewport"
...
<template>
    <div class="storyboard">
        <div ref="source" class="source hidden">
            <slot />
        </div>
        <div class="flex items-center gap-2">
            ...
            <div class="flex-1 flex items-center gap-2 justify-end-safe text-sm">
                <!-- Tabs -->
                <div class="segmented-control segmented-control-sm pill">
                    <button v-for="tab in tabs" :key="tab.key" type="button" class="segmented-control-item pill" :class="{
                ...
                <!-- Breakpoints -->
                <div class="segmented-control segmented-control-sm pill">
                    <button v-for="viewport in viewports" :key="viewport.key" type="button"
                        class="segmented-control-item pill" :class="{
                            active: activeViewport === viewport.key,
                        }" @click="setViewport(viewport.key)">
                        <i class="icon" :class="viewport.icon"></i>
                    </button>
                </div>

                <!-- Dark mode -->
                <button type="button" class="storyboard-action" @click="toggleDarkMode">
                    <i class="icon" :class="{ 'bi-moon-stars': !darkMode, 'bi-sun-fill': darkMode }"></i>
                </button>

                <!-- Copy -->
                <button v-if="activeTab === 'preview'" type="button" class="storyboard-action" title="Reload preview"
                    @click="reload">
                    <i class="icon bi-arrow-clockwise"></i>
                </button>

                <button v-else type="button" title="Copy code" class="storyboard-action" @click="copyCode">
                    <i class="icon" :class="{ 'bi-copy': !copied, 'bi-check': copied }"></i>
                </button>
            </div>
        </div>

        <!-- Preview -->
        <div v-if="activeTab === 'preview'" class="story-stage" :style="{
            minHeight: `${height}px`,
        }">
