...
interface LineNumbersPluginOptions {
  wrapperClass?: string;
  lineClass?: string;
  startLineNumber?: number;
}
export const lineNumbersPlugin = (md: MarkdownItAsync, enable = false) => {
  ...
  md.renderer.rules.fence = (...args) => {
    ...
    const lineNumbersWrapperCode = `<div class="line-numbers-wrapper pt-[48.7px]!" aria-hidden="true">${lineNumbersCode}</div>`;
